"""GeoScoutEnv — single-object NBV gym env for GeoScout.
...
# GeoScout (ShapeNet model_normalized.obj, ≈ [-0.5, 0.5]³ canonical):
...
class GeoScoutEnv(gym.Env):
    ...
    steps. GeoScout keeps GenNBV's action/reward structure while adding
    ...
    def __init__(
        ...
        if not self.sequences:
            raise ValueError("GeoScoutEnv: no sequences provided.")
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if not preproc_path.exists():
            raise FileNotFoundError(
                f"GeoScoutEnv: missing preproc {preproc_path}. "
                f"Run scripts.preprocess first."
            )
    ...
    def _dump_step_viz(self, pts_world: torch.Tensor) -> None:
        ...
        try:
            ...
            print(f"[geoscout.env] step viz failed: {type(e).__name__}: {e}")
    ...
    def _finalize_episode_dump(self) -> None:
        ...
        try:
            ...
            import traceback
            print(f"[geoscout.env] viz finalize failed: {type(e).__name__}: {e}")
            traceback.print_exc()
