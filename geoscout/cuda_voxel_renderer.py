"""Custom CUDA first-hit renderer for GeoScout voxel GT grids.

The Open3D renderer casts rays against the triangle mesh on CPU and then
copies depth/alpha back to CUDA.  For GeoScout training, the downstream
...
_EXTENSION_NAME = "geoscout_cuda_voxel_renderer_v1"
_EXTENSION = None
    ...
    with _LOAD_LOCK:
        ...
        try:
            ...
            if not _WARNED_LOAD_ERROR:
                warnings.warn(
                    "GeoScout custom CUDA voxel renderer unavailable; "
