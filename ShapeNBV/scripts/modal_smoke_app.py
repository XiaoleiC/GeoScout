"""Minimal Modal entrypoint for ShapeNBV visual smoke validation.
...
image = (
    ...
    # Keep this smoke app lean: the full ShapeNBV tree contains hundreds of MB
    # of prior visualization/report artifacts. Upload only importable code.
    .add_local_dir(
        local_path=str(Path(__file__).resolve().parents[1] / "shapenbv"),
        remote_path="/workspace/ShapeNBV/shapenbv",
    )
    .add_local_dir(
        local_path=str(Path(__file__).resolve().parents[1] / "scripts"),
        remote_path="/workspace/ShapeNBV/scripts",
    )
    .add_local_dir(
        local_path=str(Path(__file__).resolve().parents[1] / "offline_rl"),
        remote_path="/workspace/ShapeNBV/offline_rl",
    )
)
...
def probe_caption_counterfactual_policy(
    ...
    preproc_dir: str = "/data/shapenbv_preproc_g128_attr_v2_judged_v1",
    ...
    from shapenbv.tensor_env import DEFAULT_ACTION_LOW_WORLD, DEFAULT_ACTION_UNIT, NVEC

    ckpt_path = Path("/runs") / ckpt_subdir / "ppo_shapenbv.zip"
...
def evaluate_best_ckpt_numeric(
    ...
    """Numerically evaluate one selected ShapeNBV checkpoint.
    ...
    `action_space_type`, matching the 24M ShapeNBV training contract.
    """
    import os
    import subprocess
    import sys
...
def export_fql_replay_shapenet(
    ...
    preproc_dir: str = "/data/shapenbv_preproc_g128_attr_v2_judged_v1",
    hard_cases_file: str = "/workspace/ShapeNBV/offline_rl/hard_cases_judged_0509.txt",
    n_envs: int = 64,
    max_samples: int = 0,
    episode_len: int = 50,
    image_size: int = 400,
    ...
    """Export ShapeNBV rollouts as sharded FQL/offline-RL replay data."""
    import os
    import subprocess
    import sys
    ...
    cmd = [
        ...
        "--out_dir", out_dir,
        "--ckpt", f"/runs/{ckpt_subdir}/ppo_shapenbv.zip",
...
def train_fql_offline_rl(
    ...
    """Train official JAX FQL on exported ShapeNBV replay shards and eval it."""
    import os
    import subprocess
    import sys
    ...
    try:
        subprocess.check_call(
            cmd,
            env={**os.environ, "PYTHONPATH": "/workspace/ShapeNBV:/tmp/fql"},
        )
    finally:
