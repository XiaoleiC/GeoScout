...
mirrors ShapeNet's layout so `shapenbv.data.list_shapenet` works
unchanged:

    <out>/synthetic/sphere_test/models/model_normalized.obj

Usage:
    python -m scripts.make_test_mesh --out /tmp/shapenbv_smoke_data \
...
def main():
    ...
    p.add_argument("--synset", type=str, default="00000001",
                   help="Numeric folder id (must be digits — see "
                        "shapenbv.data.list_shapenet's isdigit() filter). "
