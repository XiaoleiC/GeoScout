"""Visual smoke validation for the eight ShapeNBV PPO checkpoints.
...
from shapenbv.data import SYNSET_TO_CATEGORY, list_shapenet
from shapenbv.tensor_env import (
...
DEFAULT_DISCRETE_CKPT = (
    "/runs/train600-24m-discrete-s0-l40s-n128-wandb-0506/ppo_shapenbv.zip"
)
DEFAULT_CONTINUOUS_CKPT = (
    "/runs/train600-24m-continuous-s0-l40s-n128-wandb-0506/ppo_shapenbv.zip"
)
...
def save_step_frame(
    *,
    ...
    fig.suptitle(
        f"ShapeNBV smoke validation | {row['policy']} | {row['seq_name']} | step {row['step']}",
        x=0.02,
        ha="left",
        fontsize=13,
        fontweight="bold",
...
def save_case_summary_panel(
    *,
    ...
    fig.suptitle(
        f"ShapeNBV selected-case validation | {case.get('category', '')} | {case.get('selection_reason', '')}",
        x=0.015,
        y=0.99,
        ha="left",
        fontsize=13,
...
<title>ShapeNBV selected-case visual atlas</title>
<style>
...
<h1>ShapeNBV selected-case visual atlas</h1>
...
def write_index(out_dir: Path, summaries: Sequence[dict], args) -> None:
    ...
    html_text = f"""<!doctype html>
<html><head><meta charset="utf-8">
<title>ShapeNBV 8-checkpoint smoke validation</title>
<style>
...
<body>
<h1>ShapeNBV 8-checkpoint smoke validation</h1>
...
def main() -> None:
    ...
    p.add_argument("--preproc_dir", default="/data/shapenbv_preproc_g128_attr_v2")
