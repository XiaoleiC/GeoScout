...
Usage:
    python -m shapenbv.scripts.train \
...
# stripped to ShapeNBV-relevant fields).
# ----------------------------------------------------------------------
class ShapeNBVMetricsCallback(BaseCallback):
    ...
    def _on_rollout_end(self) -> None:
        ...
        self.logger.record("shapenbv/rollout_fps", float(rollout_env_steps / dt))
        self.logger.record("shapenbv/rollout_wall_time_s", float(dt))
        self.logger.record("shapenbv/rollout_env_steps", int(rollout_env_steps))
        ...
        if callable(action_stats_fn):
            ...
            if action_stats.get("stat_steps", 0.0) > 0.0:
                self.logger.record("shapenbv/action_raw_abs_mean",
                                   float(action_stats["raw_abs_mean"]))
                self.logger.record("shapenbv/action_raw_abs_gt3_frac",
                                   float(action_stats["raw_abs_gt3_frac"]))
                self.logger.record("shapenbv/action_raw_clip_frac",
                                   float(action_stats["raw_clip_frac"]))
                self.logger.record("shapenbv/action_raw_abs_max",
        ...
        self.logger.record("shapenbv/cr_final_mean", float(crs.mean()))
        self.logger.record("shapenbv/cr_final_max", float(crs.max()))
        self.logger.record("shapenbv/cr_final_p10", float(np.percentile(crs, 10)))
        self.logger.record("shapenbv/cr_final_p25", float(np.percentile(crs, 25)))
        self.logger.record("shapenbv/cr_final_p50", float(np.median(crs)))
        self.logger.record("shapenbv/cr_final_p75", float(np.percentile(crs, 75)))
        self.logger.record("shapenbv/cr_final_p90", float(np.percentile(crs, 90)))
        self.logger.record("shapenbv/reach_cr_50_rate", float((crs >= 0.5).mean()))
        self.logger.record("shapenbv/reach_cr_80_rate", float((crs >= 0.8).mean()))
        self.logger.record("shapenbv/ep_len_mean", float(ep_lens.mean()))
        self.logger.record("shapenbv/collision_rate",
                           float(np.mean([s["collision"] for s in self._ep_stats])))
        self.logger.record("shapenbv/early_stopped_rate",
                           float(np.mean([s["early_stopped"] for s in self._ep_stats])))
        self.logger.record("shapenbv/novelty_ratio_mean",
                           float(np.mean([s["novelty_ratio"] for s in self._ep_stats])))
        self.logger.record("shapenbv/redundancy_ratio_mean",
                           float(np.mean([s["redundancy_ratio"] for s in self._ep_stats])))
        self.logger.record("shapenbv/new_gt_voxels_mean",
                           float(np.mean([s["new_gt_voxels"] for s in self._ep_stats])))
        self.logger.record("shapenbv/visible_gt_voxels_mean",
                           float(np.mean([s["visible_gt_voxels"] for s in self._ep_stats])))
        self.logger.record("shapenbv/revisit_penalty_mean",
...
class PeriodicCheckpointCallback(BaseCallback):
    ...
    which are vector-env steps rather than environment timesteps. ShapeNBV
...
def main():
    ...
    p.add_argument("--dataset", type=str, default="shapenet",
                   ...
                   help="Source dataset enumerator. `shapenet` uses "
                        "shapenbv.data.list_shapenet (ShapeNetCore.v2 "
                        "directory layout); `abo` uses shapenbv.abo."
                        "list_abo (Amazon Berkeley Objects manifest).")
    # Logging
    p.add_argument("--log_dir", type=str, default="/tmp/shapenbv_run")
    ...
    p.add_argument("--checkpoint_keep_last", type=int, default=5,
                   help="How many numbered periodic checkpoints to keep. "
                        "The ppo_shapenbv_latest.zip convenience copy is "
                        "always updated when checkpointing is enabled.")
    # Wandb
    p.add_argument("--wandb_project", type=str, default="shapenbv")
    ...
    if args.wandb_mode != "disabled":
        import datetime
        import wandb
        run_name = args.wandb_run_name or f"shapenbv-{datetime.datetime.now():%Y%m%d-%H%M%S}"
    ...
    # logger.dump() (rollout/, train/, time/, shapenbv/) to wandb.log.
    ...
    callbacks = [ShapeNBVMetricsCallback(verbose=1)]
    ...
    ckpt = log_dir / "ppo_shapenbv.zip"
    model.save(ckpt)
    print(f"[train] saved checkpoint to {ckpt}")
    if args.checkpoint_freq_steps > 0:
        checkpoint_dir = log_dir / "checkpoints"
        checkpoint_dir.mkdir(parents=True, exist_ok=True)
        latest = checkpoint_dir / "ppo_shapenbv_latest.zip"
        ...
        (checkpoint_dir / "ppo_shapenbv_latest.json").write_text(
