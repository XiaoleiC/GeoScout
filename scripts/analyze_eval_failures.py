"""Analyze GeoScout full-eval rollout results.
...
def write_summary_plot(out_path: Path, policy_stats: Sequence[dict], cat_stats: Sequence[dict], cases: Sequence[dict], threshold: float) -> None:
    ...
    fig.suptitle("GeoScout 600-sample eval: evidence summary", x=0.01, ha="left", fontweight="bold", fontsize=14)
...
def write_markdown(
    ...
    lines = [
        "# GeoScout 600-Sample Eval Failure Analysis",
        "",
...
<head>
<meta charset="utf-8">
<title>GeoScout Failure Analysis</title>
<style>
...
<body>
"""
        + f"<h1>GeoScout 600-Sample Failure Analysis</h1><p class='muted'>Sorted by learned-policy hardness. Threshold: CR &gt; {threshold:.2f}. All links are local artifacts.</p>"
        + "<div class='cards'>"
        + "\n".join(cards)
        + "</div>"
